...
Before Week 4 estimation, decide whether to proceed with the 9-10-state treatment variable now available, or block on finishing the 28 remaining timing lookups (see the coding sheet's "Open items"). Pennsylvania's county-optional cure remains a known hard case needing its own state-vs-jurisdiction-level coding convention before it enters the treatment variable either way.
...
*Part 1 (done)*: `src/06_estimate.R` first cut — naive TWFE via `fixest::feols` (jurisdiction + year FE, state-clustered) alongside `fixest::sunab()` (Sun-Abraham), the staggered-adoption-corrected estimator, with Iowa excluded from the `sunab` sample specifically since its reversal doesn't fit a single-cohort model (kept in the TWFE sample). First point estimates now exist: TWFE gives essentially a null, imprecise effect (-0.0014, SE 0.0026, p=0.59); Sun-Abraham's aggregated ATT is positive and marginally significant (+0.0037, SE 0.0018, p=0.051) — i.e. the corrected estimator moves the sign and roughly triples the precision-adjusted magnitude relative to naive TWFE, which is exactly the kind of divergence this design was built to surface, not yet a conclusion (no bootstrap SEs, no robustness checks, no heterogeneity — see below). Output: `output/tables/estimates_v1.md`.
