...
the mail-ballot transmitted/returned/counted/rejected variables by matching
their labels, and writes codebooks/crosswalk.yaml.
...
def main() -> None:
    ...
    # (C4b correlates 0.9974 with the sum of the C5* rejection-reason
    # columns and matches exactly in 94.3% of non-missing rows; C4a is
    # the *counted* total, not rejected, in this wave only).
    entries.append({
        "wave": 2016,
    ...
    print(f"{'wave':6}{'returned_by_voters':22}{'rejected_total':16}{'counted_total':16}match?")
    ok = True
    for e in entries:
        exp = expected[e["wave"]]
        match = (
        ...
        print(
            ...
            f"{'OK' if match else 'MISMATCH — check codebook label text'}"
        )
    if not ok:
        raise SystemExit(
