...
*Part 2 (done)*: `fwildclusterboot` wild-cluster bootstrap on TWFE's `treated` coefficient (B=999, clustered by state: p=0.6166, CI [-0.0069, 0.0050] — confirms the analytic SE's null); Callaway–Sant'Anna via `did::att_gt`/`aggte` as a second corrected estimator (overall ATT -0.0045, SE 0.0036, p=0.22, n=37 states — a third clean null, opposite sign from Sun-Abraham's +0.0024 but neither distinguishable from zero); heterogeneity by jurisdiction size (log ballots for TWFE, large/small tercile subsamples for Sun-Abraham/CS, substituting for the deferred ACS urban/rural split per the user's decision); and the full 5-item robustness checklist re-run through all three estimators. Outputs: `output/tables/estimates_v1.md`, `output/tables/heterogeneity.md`, `output/tables/robustness_checklist.md`.

**Two things surfaced by this round — both since investigated and diagnosed** (via `did::att_gt`'s raw group-time ATT table, `out$group/t/att/se`, available before `aggte()` aggregates it — the natural next diagnostic step beyond the aggregated numbers alone; read-only, no rerun of `06_estimate.R`, nothing below changes any number already on record):

1. **The pre-trend is real but narrow — one cohort, two states, not a design-wide flaw.** The event-time -6 signal traces to exactly one of 16 group-time cells: group=2024 (DC, MD, MI) vs. controls at t=2018, att=-0.0089, SE=0.0023. Confirmed **not a DC artifact** — DC is a single jurisdiction (n=1, high-leverage/noisy by construction), but excluding it entirely changes the estimate only from -0.0089 to -0.0092; Maryland and Michigan carry this, not DC. Confirmed **not resolved by allowing anticipation** — re-running with `anticipation=2` leaves the -6 cell significant. **Conclusion: a genuine, unexplained pre-existing difference specific to Maryland/Michigan predating their 2024 treatment by 6 years** — no mechanical fix available or attempted. Any conclusion that leans on the 2024 cohort's contribution should carry this caveat explicitly; it is not evidence the whole panel's parallel-trends assumption is broken.
2. **The heterogeneity disagreement has an identifiable mechanical cause — jurisdiction-size heterogeneity is unidentifiable with the current cohort coverage, not merely uncertain.** Two confirmed reasons, from the same raw group-time tables: (a) the **small-jurisdiction subsample is missing entire cohorts** — only the 2020 and 2022 treatment cohorts survive the size filter (9 treated states total; no 2018 or 2024 cohort at all), and one of its 8 cells is an outlier by an order of magnitude (the 2020 cohort's earliest pre-period comparison: att=-0.045, SE=0.0215, vs. every other cell in that table under 0.012 in magnitude) — small jurisdictions have volatile rates from small denominators, and this one cell dominates a thin sample; (b) the **large-jurisdiction subsample's own cohort-level effects flip sign across periods within the same cohort** (e.g., the 2024 cohort: +0.0162 at one post-period, -0.0038 at the next). Sun-Abraham's interaction-weighted averaging and Callaway-Sant'Anna's group-size-weighted averaging are not guaranteed to agree when the underlying cohort-time effects are this heterogeneous and inconsistently signed — the two estimators disagreeing here is the expected consequence of real cross-cohort heterogeneity, not a bug in either one. **Read as "not establishable with this design," not as evidence favoring either estimator's answer.**

One further wrinkle, lower stakes: the Sun-Abraham weighted-by-ballots robustness row is numerically unreliable (SE=265) — traced to `returned_by_voters`' extreme range (median ~210, max ~3.4M, a statewide-aggregate row) breaking down the interacted model's weighted variance in a way TWFE and CS tolerate but `sunab()` does not. Flagged in `robustness_checklist.md`, not silently reported as a real null.

Every other robustness/heterogeneity cell is a clean, unremarkable null — none of the 5 checklist items (drop 2020, drop WI/MI, winsorize, weight, fractional logit) flip the sign or significance of the headline result across any of the three estimators.

**Week 5 — Dashboard and packaging. Done.** `docs/index.html` (+ `app.js`, `style.css`, `docs/data/*.json`) is a self-contained static page — no build step, no Node/npm, no pandoc/quarto (none installed in this environment, which settled the "decide the hosting/build path" question from the original plan). Hosting: **GitHub Pages, served from `/docs` on `main`** — enable in repo Settings → Pages when ready; not done automatically. Verified end-to-end with headless Chrome (`--dump-dom`) rather than assumed to work: found and fixed a dead CDN path (`cdnjs.cloudflare.com/ajax/libs/plotly.js/2.35.2` 404s — cdnjs had removed that version folder; switched to `cdn.jsdelivr.net/npm/plotly.js-dist@2.35.2`) and a field-name mismatch between the hand-authored `event_study.json` and `app.js`'s reader. All 6 sections confirmed rendering real Plotly SVG content, not just placeholder text.

Content, all six sections live: (1) a plain-language headline stating the null result directly, not buried under charts; (2) coefficient comparison (TWFE/Sun-Abraham/Callaway-Sant'Anna, TWFE's wild-cluster bootstrap CI shown alongside its analytic one); (3) event study with the diagnosed pre-trend cell (event-time -6, driven by DC/MD/MI's 2024 cohort) visually flagged red and explained in place, not hidden; (4) rejection-rate distribution by state, wave-selectable; (5) county choropleth; (6) a written limitations section covering both diagnosed Week 4 caveats plus the standing project caveats (IL/MA/OH/MT confidence, PA/NC design decisions, deferred ACS, coding-sheet staleness).

**The county choropleth's scope required a real decision, made explicitly rather than assumed away**: EAVS jurisdictions are counties in most states, but 7 states report at the town/municipality level instead — Wisconsin (1,861 jurisdictions vs. 72 real counties, confirmed via a synthetic all-zero FIPS prefix with no county encoding at all) and the 6 New England states (ratios of 8:1 to 32:1 jurisdiction-count to county-FIPS-prefix-count). Verified the join mechanism directly: for a normal county-reporting state, the panel's 10-digit `fips` string's first 5 characters ARE the real Census county FIPS (`0603700000` → `06037` = Los Angeles County, confirmed). Per the decision made with the user: map the 44 states + DC that report at true county granularity accurately, and gray out the 7 town-reporting states with an explicit legend note — rather than build an unvalidated town-to-county crosswalk (real new scope) or reduce the whole map to state-level (a real capability loss for the 44 states that do have county detail).

Estimation-result JSON (`model_comparison.json`, `event_study.json`, `heterogeneity.json`, `robustness.json`) was hand-authored from already-verified numbers rather than forcing another ~25-30 min rerun of `06_estimate.R` — but `06_estimate.R` itself was extended with equivalent `jsonlite`-based export code (syntax-checked, not executed this round) so a future actual rerun regenerates these files automatically and they can't silently drift from the script that's supposed to produce them.
